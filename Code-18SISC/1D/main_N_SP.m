...
% load exact solution
filename=strcat('MGPE-SP1d-Bet-',int2str(beta),'-Del-',int2str(delta),'-Vep-',num2str(mvep),'-NN-',int2str(NN_ex),'_n.mat');
load(filename)
...
for md_N=1:NN_len_N
    NN=NN_list_N(md_N);    
    filename=strcat('MGPE-SP1d-Bet-',int2str(beta),'-Del-',int2str(delta),'-Vep-',num2str(mvep),'-NN-',int2str(NN),'_n.mat');
%     disp(filename)
    load(filename)
    x=data.x;
    Rho_test=finer_SP(Rho(2:end-1),NN_ex-NN); % Be careful: in whole space problem, boundary might be not 0

    E_err(md_N)=abs(E-E_ex);
    l2_err(md_N)=sqrt(sum(h_ex*(Rho_test-Rho_ex).^2));
    h1_err(md_N)=sqrt(sum(diff(Rho_test-Rho_ex).^2)/h_ex); % inaccurate for SP
