...
for dvep = 1:veplen 
    ...
    data.vep = vep;        
        for dN = 1:Nlen        
            ...
            [E,Rho] = MGPE_SP1d_Exam1(data,vep,NN,X0);           
            disp(E)
            
            filename = strcat('MGPE-SP1d-Bet-',int2str(data.beta),'-Del-',int2str(data.delta),'-Vep-',num2str(vep),'-NN-',int2str(NN),'_n.mat');
            save(filename)
        end
